...
static void spi_stm32_dma_stop(const struct device *dev);
			 ...
			 uint32_t channel, int status)
{
	...
	if ((data->status_flags & SPI_STM32_DMA_DONE_FLAG) == data->all_done) {
		...
		} else if (!IS_ENABLED(CONFIG_SPI_STM32_INTERRUPT)) {
			spi_stm32_complete(dev, 0);
		}
	}
}
...
static void spi_stm32_complete(const struct device *dev, int status)
{
	...
	if (IS_ENABLED(CONFIG_SPI_STM32_INTERRUPT) || IS_ENABLED(CONFIG_SPI_STM32_DMA)) {
...
static size_t spi_stm32_dma_len(struct spi_stm32_data *data) {
	size_t dma_len;
